function [W, stat, primal_gap_iter] = gsp_learn_graph_log_degrees_runtime(Z, a, b, alpha, beta, stepsize, max_iter, eps, W_opt, params)
...
if not(isfield(params, 'verbosity')),   params.verbosity = 4;   end
if not(isfield(params, 'maxit')),       params.maxit = max_iter;      end
if not(isfield(params, 'tol')),         params.tol = eps;      end
if not(isfield(params, 'step_size')),   params.step_size = stepsize;      end     % from (0, 1)
...
for i = 1:params.maxit
    ...
    %% (commented when comparing runtime)
...
%     end
    ...
    %% added by xlwang
    W = sqrt(alpha/beta)*squareform_sp(w);
    primal_gap_iter(i) = norm((W-W_opt),'fro')/sqrt(2); % commented when comparing runtime
    
    %%
    if primal_gap_iter(i) < eps
        break
    end
end
stat.time = toc;
