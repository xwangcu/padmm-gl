function [W, fval_admm, fval_iter, primal_gap_iter] = dgl_tikhonov_admm(X, alpha, beta, gamma, t, tau1, tau2, max_iter, epsilon, w_opt,T)
...
%     end
    ...
    fval_iter(k) = 2*(z')*w - alpha*sum(log(v_1)) + gamma*norm(v_2,2)^2 + beta*(w')*w; % commented when comparing runtime
