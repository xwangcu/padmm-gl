...
fprintf('solving...\n');
tic
[w_opt, fval_admm_iter_opt, primal_res_iter_opt] = dgl_tikhonov_admm_solver(X_noisy, alpha, beta, gamma, t, tau1, tau2, max_iter_opt, epsilon_opt, time_slots);
toc
...
tau2 = 1e-4;
tic
[W_admm, fval_admm, fval_admm_iter, primal_gap_iter_admm] = dgl_tikhonov_admm(X_noisy, alpha, beta, gamma, t, tau1, tau2, max_iter, epsilon, w_opt, time_slots);
admm_time = toc;
...
[W_pds, fval_pds, primal_gap_iter_pds, ~] = tvglearn_tikhonov(z_all, alpha, beta, gamma, w_opt, params);
pds_time = toc;
...
lgd.FontSize = 15;
title('$L_{1,1}$-norm regularizer','Interpreter','latex','FontSize',20);
% title('Tikhonov regularizer','Interpreter','latex','FontSize',20);
