function [W_all, stat] = tvglearn_grouplasso(Z, alpha, beta, eta, params)

% Estimate a time-varying graph from a spatial-time series signal.
...
if not(isfield(params, 'maxit')),       params.maxit = 10000;      end
if not(isfield(params, 'tol')),         params.tol = 1e-3;      end
if not(isfield(params, 'step_size')),   params.step_size = .5;      end     % from (0, 1), in otehr words, 0<step_size<1
...
function[group_norm] = compute_group_norm(x, T)
X = reshape(x, [], T);
group_norm = sum(sqrt(sum(X.*X)));
end
