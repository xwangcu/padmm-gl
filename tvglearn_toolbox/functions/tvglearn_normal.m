function [W_all, stat] = tvglearn_normal(Z, alpha, beta, params)
...
if w_0 == 0
    ...
    h.eval = @(w) beta * sum(vecnorm(w).^2);
    h.grad = @(w) 2 * beta * w;
    h.beta = 2 * beta;
else
    w_0 = reptmat(w_0, [1 T]);
    h.eval = @(w) beta * sum(vecnorm(w).^2) + c *  sum(vecnorm(w - w_0).^2);
